...
void POSITION::do_color(bool color_id) noexcept {
    assert(m_turn == player2); // 前の手番がplayer1のときしか駒の公開は生まれない（player1にとって）
    if(m_last_capture_p1 == (purple|player2)) return; // 公開可能な駒がないときは何もせず返す
    
    if(color_id == 0) { // idが0のときは青色の駒を取ったことにする
        num_eb--;
        m_last_capture_p1 = (blue|player2);
    } else if(color_id == 1) { // idが1のときは赤色の駒を取ったことにする
        num_er--;
...
void POSITION::change_player(unsigned char array_sq[36]) noexcept {
    for(int i = 0; i < 40; i++) {
        if((array_sq[i] & 1U) == player1 && array_sq[i] != 0U) array_sq[i] = array_sq[i] + 1U;
        else if((array_sq[i] & 1U) == player2) array_sq[i] = array_sq[i] - 1U;
    }
    // unsigned int new_last_capture_p1, new_last_capture_p2;
    // if(m_last_capture_p1 != 0U) new_last_capture_p2 = m_last_capture_p2 - 1U;
    // else new_last_capture_p2 = 0U;
    // if(m_last_capture_p2 != 0U) new_last_capture_p1 = m_last_capture_p1 + 1U;
    // else new_last_capture_p1 = 0U;
    // m_last_capture_p1 = new_last_capture_p1, m_last_capture_p2 = new_last_capture_p2;
