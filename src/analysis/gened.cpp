...
unsigned long long int nwin = 0, nlose = 0, ncan_lose = 0, nunknown = 0, lunknown; // lunknownが最後のunknownの番号(-1しておく)
...
ZDD& ZDD::get(int iter, int i, int j, int k, int l) noexcept {
    int e = k+l;
    if(iter % 2 == 1) {
        static ZDD inst(i, j, e), inst_e(i, j, e-1); // private なコンストラクタを呼び出す。
...
Database& Database::get(int iter, int i, int j, int k, int l) noexcept {
        ...
        if(iter % 2 == 1) { // 手番プレイヤが1なら次の3つの表を読む
            ...
            string file_name1 = "./table/table" + to_string(i) + '-' + to_string(j) + '-' + to_string(k) + '-' + to_string(l) + "_p1.bin";
            static Database table(file_name1, ZDD::get(iter, i, j, k, l).get_num());
            unsigned long long int size1, size2;
            // if (i < 2) size1 = 0;
            // else size1 = ZDD::get(iter, i-1, j, k, l).get_num();
            // if (j < 2) size2 = 0;
            // else size2 = ZDD::get(iter, i, j-1, k ,l).get_num();
            string file_name2 = "./table/table" + to_string(i-1) + '-' + to_string(j) + '-' + to_string(k) + '-' + to_string(l) + "_p1.bin";
            static Database table_i(file_name2, ZDD::get(iter, i-1, j, k, l).get_num());
            string file_name3 = "./table/table" + to_string(i) + '-' + to_string(j-1) + '-' + to_string(k) + '-' + to_string(l) + "_p1.bin";
            static Database table_j(file_name3, ZDD::get(iter, i, j-1, k ,l).get_num());
            if     (i == NUM_B   && j == NUM_R   && k == NUM_EB && l == NUM_ER) return table;
            else if(i == NUM_B-1 && j == NUM_R   && k == NUM_EB && l == NUM_ER) return table_i;
            else if(i == NUM_B   && j == NUM_R-1 && k == NUM_EB && l == NUM_ER) return table_j;
            else {
    ...
    Table(int iter, string read_file_name) noexcept : m_table(new uint64_t [table_size64_2] ){
        ...
        if(!read_file) {    //readファイルがなかった場合
            ...
            {   
                ...
                std::cout << "before nwin  =  " << nwin << endl;
                #ifndef USE_PURPLE
                    std::cout << "before ncan_lose  =  " << ncan_lose << endl;
                #endif
...
private:
    ...
    unsigned int m_color[max_legal_num];  //子節点で公開可能な駒があるか否か
...
public:
    ...
    void set_array_val(int num_child, unsigned int color[], unsigned int array_val[][2]) noexcept {
        m_num_child = num_child;
        assert(m_num_child > 0 && m_num_child <= max_legal_num);
        for(int childid = 0; childid < m_num_child; childid++) {
            m_color[childid] = color[childid];
            assert(m_color[childid] == 1 || m_color[childid] == 2);
            for(int colorid = 0; colorid < m_color[childid]; colorid++) { // 配列の最後に青赤の分岐があるかを格納
    ...
    void get_color(unsigned int color[]) const noexcept {
        assert(m_num_child <= max_legal_num);
        for(int childid = 0; childid < m_num_child; childid++) {
            color[childid] = m_color[childid];
        }
    }
    void get_array(unsigned int color[], unsigned int array_val[][2]) const noexcept {
        assert(m_num_child > 0 && m_num_child <= max_legal_num);
        for(int childid = 0; childid < m_num_child; childid++) {
            color[childid] = m_color[childid];
            assert(m_color[childid] == 1 || m_color[childid] == 2);
            for(int colorid = 0; colorid < m_color[childid]; colorid++) {
    ...
    while(true) {// 仕事がなくなるまで繰り返す
        ...
        if ((deq_input.size() < deq_input_size) && (deq_output.size() < deq_output_size) && count_input <= lunknown) {
            ...
            while(table.get(count_input) != v_unknown) {    //unknownが見つかるまで
                ...
                #ifdef USE_PURPLE
                    if(count_input % 1000000000ULL == 0ULL) std::cout << "count_input: " << count_input << ", nwin: " << nwin << ", nlose: " << nlose << ", nunknown: " << nunknown << endl;
                #else
                    if(count_input % 1000000000ULL == 0ULL) std::cout << "count_input: " << count_input << ", nwin: " << nwin << ", ncan_lose: " << ncan_lose << ", nlose: " << nlose << ", nunknown: " << nunknown << endl;
                #endif
            ...
            if (0 < deq_output.size()) {
                ...
                for (unsigned int workid = 0; workid < deq_tmp.size(); workid++) {
                    ...
                    int nchild = deq_tmp[workid]->get_num_child();          //そのidの子供の数(合法手の数)を得る
                    if (nchild > 0) {   //子供がいれば(いないとおかしい)
                        ...
                        deq_tmp[workid]->get_array(color, array);
                            ...
                            // cout << "workid : " << id << endl;
                            if(iter % 2 == 1) {
                                is_win = false, is_can_lose = true;
                                for(int childid = 0; childid < nchild; childid++) {
                                    bool is_wins = true, is_can_loses = false;
                                    for(int colorid = 0; colorid < color[childid]; colorid++) {
                                        if(array[childid][colorid] != v_win) {
                                            is_wins = false;
                                            if(array[childid][colorid] != v_unknown) is_can_loses = true;
                                        }
                                    }
                                    if(is_wins) {
                                        is_win = true;
                                    ...
                                    if(!is_can_loses) is_can_lose = false;
                                }
                            } else if(iter % 2 == 0) {
                                is_win = true, is_can_lose = false;
                                for(int childid = 0; childid < nchild; childid++) {
                                    assert(color[childid] == 1);
                                    if(array[childid][0] == v_lose || array[childid][0] == v_can_lose) {
                                        is_win = false;
                                        is_can_lose = true;
                                        break;
                                    } else if(array[childid][0] == v_unknown) {
                                        is_win = false;
                                    }
                                }
                            }
                            ...
                            } else if (is_can_lose) {        //負けありをセット
                                table.set(id, v_can_lose);
                                count_changes++;
                                ncan_lose++;
                                nunknown--;
                                os.seekp(id, ios::beg);  //手数の表のi番目を書き換える
...
int main(int argc, char *argv[]) {
    ...
    // max_table_size = ZDD::get(0, 1, 1, 1, 1).get_num();
    // string read_file_name = "./table/table1-1-1-1_p2.bin";
    // Table table(0, read_file_name); //メモリにtableを作る
    ...
    #ifdef USE_PURPLE
        string read_file_name = "./table_purple/table" + to_string(NUM_B) + '-' + to_string(NUM_R) + '-' + to_string(NUM_EB) + '-' + to_string(NUM_ER) + '_' + player + ".bin";
        string write_file_name = "./db_purple/db" + to_string(NUM_B) + '-' + to_string(NUM_R) + '-' + to_string(NUM_EB) + '-' + to_string(NUM_ER) + '_' + player + ".bin";
    #else
    ...
    if(atoi(argv[1]) == 1) {
        ...
        #else
            string read_file_name2 = "./table/table" + to_string(NUM_B) + '-' + to_string(NUM_R) + '-' + to_string(NUM_EB) + '-' + to_string(NUM_ER) + "_p2" + ".bin";
            string write_file_name2 = "./db/db" + to_string(NUM_B) + '-' + to_string(NUM_R) + '-' + to_string(NUM_EB) + '-' + to_string(NUM_ER) + "_p2" + ".bin";
        #endif
    ...
    std::cout << "after nwin  =  " << nwin << endl;
    #ifndef USE_PURPLE
        std::cout << "after ncan_lose  =  " << ncan_lose << endl;
    #endif
