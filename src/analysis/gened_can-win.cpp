...
unsigned long long int ncan_win = 0, nunknown = 0, lunknown; // lunknownが最後のunknownの番号(-1しておく)
...
Database& Database::get(int iter, int i, int j, int k, int l, bool flip) noexcept { // flipは盤面をひっくり返しているか
    assert(i > 0 && j > 0 && k > 0 && l > 0);
    // cout << iter << " : " << i << " : " << j << " : " << k << " : " << l << endl;
    #ifdef USE_PURPLE
        static Database table(i, j, k, l), table_i(i-1, j, k, l), table_j(i, j-1, k, l),
                        table_l(i, j, k, l-1), table_i_l(i-1, j, k, l-1), table_j_l(i, j-1, k, l-1);
        assert(k == NUM_EB);
        if     (i == NUM_B   && j == NUM_R   && l == NUM_ER)    return table;
        else if(i == NUM_B   && j == NUM_R   && l == NUM_ER-1)  return table_l;
        else if(i == NUM_B-1 && j == NUM_R   && l == NUM_ER)    return table_i;
        else if(i == NUM_B-1 && j == NUM_R   && l == NUM_ER-1)  return table_i_l;
        else if(i == NUM_B   && j == NUM_R-1 && l == NUM_ER)    return table_j;
        else if(i == NUM_B   && j == NUM_R-1 && l == NUM_ER-1)  return table_j_l;
        else {
            cerr << "no file: " << i << ", " << j << ", " << k << ", " << l << endl;
            terminate();
        }
    #else
        if(iter % 2 == 1) { // 手番プレイヤが1なら次の3つの表を読む
            string file_name1 = "./table/table_cw" + to_string(i) + '-' + to_string(j) + '-' + to_string(k) + '-' + to_string(l) + "_p2.bin";
            static Database table(file_name1, ZDD::get(iter, i, j, k, l).get_num());
            string file_name4 = "./table/table" + to_string(k) + '-' + to_string(l) + '-' + to_string(i) + '-' + to_string(j) + "_p1.bin";
            static Database table_flip(file_name4, ZDD::get(iter, k, l, i, j).get_num());
            unsigned long long int size1, size2, size3, size4;
            // if (k < 2) size1 = 0, size3 = 0;
            // else size1 = ZDD::get(iter, i, j, k-1, l).get_num(), size3 = ZDD::get(iter, k-1, l, i, j).get_num();
            // if (l < 2) size2 = 0, size4 = 0;
            // else size2 = ZDD::get(iter, i, j, k ,l-1).get_num(), size4 = ZDD::get(iter, k, l-1, i, j).get_num();
            string file_name2 = "./table/table_cw" + to_string(i) + '-' + to_string(j) + '-' + to_string(k-1) + '-' + to_string(l) + "_p2.bin";
            static Database table_k(file_name2, ZDD::get(iter, i, j, k-1, l).get_num());
            string file_name5 = "./table/table" + to_string(k-1) + '-' + to_string(l) + '-' + to_string(i) + '-' + to_string(j) + "_p1.bin";
            static Database table_kflip(file_name5, ZDD::get(iter, k-1, l, i, j).get_num());
            string file_name3 = "./table/table_cw" + to_string(i) + '-' + to_string(j) + '-' + to_string(k) + '-' + to_string(l-1) + "_p2.bin";
            static Database table_l(file_name3, ZDD::get(iter, i, j, k ,l-1).get_num());
            string file_name6 = "./table/table" + to_string(k) + '-' + to_string(l-1) + '-' + to_string(i) + '-' + to_string(j) + "_p1.bin";
            static Database table_lflip(file_name6, ZDD::get(iter, k, l-1, i, j).get_num());
            
            if     (i == NUM_B && j == NUM_R && k == NUM_EB   && l == NUM_ER && !flip)    return table;
            else if(i == NUM_EB && j == NUM_ER && k == NUM_B && l == NUM_R && flip) return table_flip;
            else if(i == NUM_B && j == NUM_R && k == NUM_EB-1 && l == NUM_ER && !flip)    return table_k;
            else if(i == NUM_EB-1 && j == NUM_ER && k == NUM_B && l == NUM_R && flip) return table_kflip;
            else if(i == NUM_B && j == NUM_R && k == NUM_EB   && l == NUM_ER-1 && !flip)  return table_l;
            else if(i == NUM_EB && j == NUM_ER-1 && k == NUM_B && l == NUM_R && flip) return table_lflip;
            else {
                cerr << "no file: " << i << ", " << j << ", " << k << ", " << l << ", player2" << endl;
                std::terminate();
            }
        } else { // 手番プレイヤが2なら次の3つの表を読む
            string file_name1 = "./table/table_cw" + to_string(i) + '-' + to_string(j) + '-' + to_string(k) + '-' + to_string(l) + "_p1.bin";
            static Database table(file_name1, ZDD::get(iter, i, j, k, l).get_num());
            string file_name4 = "./table/table" + to_string(k) + '-' + to_string(l) + '-' + to_string(i) + '-' + to_string(j) + "_p2.bin";
            static Database table_flip(file_name4, ZDD::get(iter, k, l, i, j).get_num());
            unsigned long long int size1, size2, size3, size4;
            // if (i < 2) size1 = 0, size3 = 0;
            // else size1 = ZDD::get(iter, i-1, j, k, l).get_num(), size3 = ZDD::get(iter, k, l, i-1, j).get_num();
            // if (j < 2) size2 = 0, size4 = 0;
            // else size2 = ZDD::get(iter, i, j-1, k ,l).get_num(), size4 = ZDD::get(iter, k, l, i, j-1).get_num();
            string file_name2 = "./table/table_cw" + to_string(i-1) + '-' + to_string(j) + '-' + to_string(k) + '-' + to_string(l) + "_p1.bin";
            static Database table_i(file_name2, ZDD::get(iter, i-1, j, k, l).get_num());
            string file_name5 = "./table/table" + to_string(k) + '-' + to_string(l) + '-' + to_string(i-1) + '-' + to_string(j) + "_p2.bin";
            static Database table_iflip(file_name5, ZDD::get(iter, k, l, i-1, j).get_num());
            string file_name3 = "./table/table_cw" + to_string(i) + '-' + to_string(j-1) + '-' + to_string(k) + '-' + to_string(l) + "_p1.bin";
            static Database table_j(file_name3, ZDD::get(iter, i, j-1, k ,l).get_num());
            string file_name6 = "./table/table" + to_string(k) + '-' + to_string(l) + '-' + to_string(i) + '-' + to_string(j-1) + "_p2.bin";
            static Database table_jflip(file_name6, ZDD::get(iter, k, l, i, j-1).get_num());
            
            if     (i == NUM_B && j == NUM_R && k == NUM_EB   && l == NUM_ER && !flip)    return table;
            else if(i == NUM_EB && j == NUM_ER && k == NUM_B && l == NUM_R && flip) return table_flip;
            else if(i == NUM_B-1 && j == NUM_R && k == NUM_EB && l == NUM_ER && !flip)    return table_i;
            else if(i == NUM_EB && j == NUM_ER && k == NUM_B-1 && l == NUM_R && flip) return table_iflip;
            else if(i == NUM_B && j == NUM_R-1 && k == NUM_EB   && l == NUM_ER && !flip)  return table_j;
            else if(i == NUM_EB && j == NUM_ER && k == NUM_B && l == NUM_R-1 && flip) return table_jflip;
            else {
                cerr << "no file: " << i << ", " << j << ", " << k << ", " << l << ", player1" << endl;
                std::terminate();
            }
        }
    #endif
...
private:
    ...
    bool m_is_can_win; // ただただ勝有であるかどうかだけを格納するブール代数
    unsigned int m_action[max_legal_num];  //子節点で公開可能な駒があるか否か, 繰り返し奇数回のときは各belief stateで子節点がいくつあるか
    unsigned int m_array[max_belief_state][max_legal_num][2]; //各値を保持する
...
public:
    ...
    void set_cw(int is_can_win) noexcept { m_is_can_win = is_can_win; }
    void set_array_val(unsigned int state, unsigned int action[], unsigned int array[][max_legal_num][2]) noexcept {
        m_state = state;
    ...
    bool get_cw() const noexcept { return m_is_can_win; }
    ...
    void get_array(unsigned int action[], unsigned int array[][max_legal_num][2]) const noexcept {
        assert(m_state >= 2 && m_state <= max_belief_state);
        for(int stateid = 0; stateid < m_state; stateid++) {
            action[stateid] = m_action[stateid];
...
static void boss(int iter, string read_file_name, string write_file_name) noexcept {
    ...
    unsigned long long int count_newwins = 0ULL; //新しい必勝の数
        ...
        if ((deq_input.size() < deq_input_size) && (deq_output.size() < deq_output_size) && count_input <= lunknown) {
            ...
            if (0 < deq_output.size()) {
                ...
                for (unsigned int workid = 0; workid < deq_tmp.size(); workid++) {
                    ...
                    bool is_can_win = deq_tmp[workid]->get_cw(); // 勝有か否かを読む
                    // unsigned int nstate = deq_tmp[workid]->get_num_state();          //そのidの子供の数(合法手の数)を得る
                    // assert(nstate >= 2); // belief stateは2個以上あるはず
                    // unsigned int naction[nstate], array[nstate][max_legal_num][2];
                    // deq_tmp[workid]->get_array(naction, array);
                    #ifdef USE_PURPLE
                        bool is_win = false;
                        ...
                        //             is_can_win = true;
                        //             if(array[stateid][actionid][0] != v_win) {
                        //                 is_can_win = false; // 1個でも勝ちの配置ならそれはもう勝有
                        //                 continue; // で、break
                        //             }
                        //             if(array[stateid][actionid][1] != v_can_lose && array[stateid][actionid][1] != v_lose) {
                        //                 is_can_win = false;
                        //                 continue;
                        //             }
                        //             break;
                        ...
                        //     // if(is_can_win) cout << "もろたで工藤" << endl;
                        // }
...
static void worker(int iter) noexcept {
    ...
    while (true) { //仕事を全て終えるまで繰り返す
        ...
        assert(nstate >= 2);
        unsigned int result;  // 配置のインデックスをとにかく格納
        unsigned int naction; // 各物理状態の行動数
        if(iter % 2 == 1) {
            for(int stateid = 0; stateid < nstate; stateid++) {
                posi.do_belief_state(combinations[stateid]);
                Action actions[max_legal_num];
                naction = posi.gen_actions(actions); // 合法手列挙
                assert(naction > 0);
                for(int actionid = 0; actionid < naction; actionid++) {
                    is_can_win = true;
                    posi.do_action(actions[actionid]); // 行動実行
                    if(posi.is_end() != 0) result = posi.is_end();
                    else result = Database::get(iter, posi.get_blue(), posi.get_red(), posi.get_enemy_blue(), posi.get_enemy_red(), false).read_database(posi.compute_id());
                    if(result != v_win) {
                        is_can_win = false; // 勝有でない配置が見つかったなら次のループに入ってよし
                        posi.undo_action(); // 行動を戻すのは忘れずに
                        continue;
                    }
                    posi.do_flip();
                    if(posi.is_end() != 0) result = posi.is_end();
                    else result = Database::get(iter, posi.get_blue(), posi.get_red(), posi.get_enemy_blue(), posi.get_enemy_red(), true).read_database(posi.compute_id());
                    posi.undo_flip();
                    posi.undo_action();
            ...
            for(int stateid = 0; stateid < nstate; stateid++) {
                ...
                for(int actionid = 0; actionid < naction; actionid++) {
                    ...
                    if(result != v_win) {
                        is_can_win = false; // 相手の全ての行動に対して勝有でないといけないのでbreak
                        posi.undo_action(); // 行動を戻すのは忘れずに
                        break;
                    }
                    posi.do_flip();
        ...
        w->set_cw(is_can_win); // ここで計算し切るつもりなら必要なのは計算結果ただ一つ

        // 結局ここで計算し切るほうが一番計算コストが安い
        // Combination combinations[max_belief_state];
        // int nstate = posi.gen_belief_state(combinations);
        // assert(nstate >= 2);
        ...
        //     for(int actionid = 0; actionid < naction[stateid]; actionid++) {
        //         posi.do_action(actions[actionid]); // 行動実行
        //         if(posi.is_end() != 0) array[stateid][actionid][0] = posi.is_end();
        //         else array[stateid][actionid][0] = Database::get(iter, posi.get_blue(), posi.get_red(), posi.get_enemy_blue(), posi.get_enemy_red(), false).read_database(posi.compute_id());
        //         posi.do_flip();
        //         if(posi.is_end() != 0) array[stateid][actionid][1] = posi.is_end();
        //         else array[stateid][actionid][1] = Database::get(iter, posi.get_blue(), posi.get_red(), posi.get_enemy_blue(), posi.get_enemy_red(), true).read_database(posi.compute_id());
        //         posi.undo_flip();
        //         posi.undo_action();
        //     }
        //     posi.undo_belief_state();
        // }
        // w->set_array_val(nstate, naction, array); // 必要な情報をすべて記録
...
int main(int argc, char *argv[]) {
    ...
    Database::get(atoi(argv[1]), NUM_B, NUM_R, NUM_EB, NUM_ER, false).out_info();
