...
public:
    ...
    POSITION(unsigned char i, unsigned char j, unsigned char k, unsigned char l, unsigned long long int id, unsigned int turn) noexcept : m_turn(turn), m_last_action_p1(Action()), m_last_capture_p1(0),
                                m_last_action_p2(Action()), m_last_capture_p2(0), last_flip(Combination()),
    ...
    POSITION(int iter, unsigned char i, unsigned char j, unsigned char k, unsigned char l, unsigned long long int id) noexcept : iter(iter), m_last_action_p1(Action()), m_last_capture_p1(0),
                                m_last_action_p2(Action()), m_last_capture_p2(0), last_flip(Combination()),
                                num_b(i), num_r(j), num_eb(k), num_er(l), now_id(id) {
    ...
    unsigned char get_capture() const noexcept {
        if(m_turn == player1) {
            if(m_last_capture_p2 == (blue|player1)) return blue;
            else if(m_last_capture_p2 == (red|player1)) return red;
            else return 0U;
        } else if(m_turn == player2) {
            if(m_last_capture_p1 == (blue|player2)) return blue;
            else if(m_last_capture_p1 == (red|player2)) return red;
            else return 0U;
        } else std::terminate();
    }
    ...
    uint8_t num_unveil() const noexcept {   // 駒公開の際の分岐数を返す
        if(m_last_capture_p1 == (purple|player2)) return 2;
        else return 1;
    }
    void do_color(bool color_id) noexcept;              // 公開可能な色を公開する
    ...
    bool is_goal() const noexcept {   // いずれかの青駒がゴールしているかを返す関数
        if(array_sq[36] == (blue|player2) || array_sq[36] == (purple|player2) ||
                array_sq[37] == (blue|player2) || array_sq[37] == (purple|player2) ||
                array_sq[38] == (blue|player1) || array_sq[39] == (blue|player1)) return true;
        return false;
    }
